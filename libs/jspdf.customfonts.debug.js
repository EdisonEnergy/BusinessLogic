  ...
  return /******/ (function(modules) {
          ...
          args.text = isHex
            ? text.map(function(str) {
                return Array.isArray(str)
                  ? [encode(subset, str[0]), str[1], str[2]]
                  : encode(subset, str);
              })
            : text;
    ...
    */
            function makeFontTable(data) {
              ...
              if (data.Type === "Font") {
                if (isHex)
                  data.ToUnicode = makeFontTable(data.ToUnicode) + " 0 R";
                data.FontDescriptor =
                  makeFontTable(data.FontDescriptor) + " 0 R";
          ...
          var b64ToByteArray = function b64ToByteArray(b64) {
            ...
            placeHolders =
              "=" === b64.charAt(len - 2)
                ...
                : "=" === b64.charAt(len - 1)
                ? 1
                : 0; // base64 is 4/3 + up to two characters of the original data
          ...
          CmapEntry.encode = function(charmap, encoding) {
            ...
            switch (encoding) {
              ...
              case "unicode":
                ...
                for (
                  i = _k = 0, _len2 = startCodes.length;
                  _k < _len2;
                  i = ++_k
                ) {
          ...
          function(font) {
            ...
            if (jsPDF.API.existsFileInVFS(postScriptName)) {
              font.metadata = jsPDF.API.TTFFont.open(
                postScriptName,
                fontName,
                jsPDF.API.getFileFromVFS(postScriptName)
              );
              var _font$metadata = font.metadata,
                widths = _font$metadata.hmtx.widths,
                capHeight = _font$metadata.capHeight;
              font.encoding =
                widths.length < 500 && capHeight < 800
                  ? "WinAnsiEncoding"
                  : "MacRomanEncoding";
            } else if (id.slice(1) >= 15) {
              console.error(
                "Font does not exist in FileInVFS, import fonts or remove declaration doc.addFont('".concat(
                  postScriptName,
                  "')."
                )
              );
